...
readonly output_root="${research_root}/results/channel_opex_dev_extension"
...
readonly protocol="${code_dir}/channel_opex_boundary_extension_protocol.json"

declare -A output_names=(
  ["inverse:0.3"]="inverse_t2_k8_d025_eta03_seed0_dev10"
  ["inverse:1.0"]="inverse_t2_k8_d025_eta1_seed0_dev10"
  ["identity:0.3"]="identity_t2_k8_d2_eta03_seed0_dev10"
  ["identity:1.0"]="identity_t2_k8_d2_eta1_seed0_dev10"
)
...
"${python_bin}" select_channel_opex_boundary_extension.py \
  --protocol "${protocol}" \
  --output "${output_root}/selection.json"
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) CA-OPEX boundary extension selection complete"
